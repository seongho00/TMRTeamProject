...
for filename in os.listdir(input_folder):
    if filename.endswith(".csv"):
        ...
        for start, end in age_ranges:
            ...
            # ✅ 60~200세 범위에는 110세이상 컬럼 수동 추가
            if start == 60 and end == 200:
                if "110세이상 남자" in daejeon_df.columns:
                    male_cols.append("110세이상 남자")
                if "110세이상 여자" in daejeon_df.columns:
                    female_cols.append("110세이상 여자")
        ...
        cols_to_drop = [col for col in daejeon_df.columns if '세남자' in col or '세여자' in col or '110세이상 남자' in col or '110세이상 여자' in col or '기준연월' in col]

...
import com.koreait.exam.tmrteamproject.service.NotificationService;
    ...
    public void checkAndSendSms() {
        ...
        for (ScheduleInterest si : scheduleInterests) {
            ...
            if (msg != null && si.getIsActive() == 1) {
                ...
                // 웹 팝업 알림창
                DueAlert dueAlert = DueAlert.builder()
                        .scheduleId(lhSupplySchedule.getId())
                        .name(lhSupplySchedule.getName())
                        .applyStart(lhSupplySchedule.getApplyStart())
                        .daysUtil((int) daysUntil)
                        .msg(msg)
                        .build();

                notificationService.notifyMember(si.getMemberId(), dueAlert);
                log.info("웹 알림 전송 완료 -> memberId={}, msg={}", si.getMemberId(), msg);
            }
        }
